...
int main()
{
    ...
    float vertices[] = {
        0.5f,  0.5f, 0.0f,  // 右上角 top right
        0.5f, -0.5f, 0.0f,  // 右下角 bottom right
        -0.5f, -0.5f, 0.0f,  // 左下角 bottom left
        -0.5f,  0.5f, 0.0f   // 左上角 top left
    };
    ...
    while (!glfwWindowShouldClose(window)) //每次循环的开始前检查一次GLFW是否被要求退出
    {
        ...
        float greenValue = sin(timeValue) / 2.0f + 0.5f; // 0～1 之间变化
        int vertexColorLocation = glGetUniformLocation(shaderProgram, "ourColor"); // 得到ourColor的位置值；参数：着色器程序和uniform的名字
        glUniform4f(vertexColorLocation, 0.0f, greenValue, 0.0f, 1.0f); //设置uniform值 vec4(0.0f, greenValue, 0.0f, 1.0f)
