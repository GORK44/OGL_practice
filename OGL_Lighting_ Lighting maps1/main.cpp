...
//    glEnableVertexAttribArray(2); //启用顶点属性2（顶点的 属性位置 = 2）
    ...
    // ----------------------------------------------------------
    unsigned int diffuseMap; //纹理ID
    
    // texture 1
    // ----------------------------------
    glGenTextures(1, &diffuseMap); //生成 1 个纹理，保存ID到texture
    glBindTexture(GL_TEXTURE_2D, diffuseMap); // 绑定纹理，接下来所有GL_TEXTURE_2D操作都是对此纹理
...
//    stbi_set_flip_vertically_on_load(true); // 让stb_image.h在加载图片时翻转y轴
    unsigned char *data = stbi_load("/书/OGL_Test/container2.jpg", &width, &height, &nrChannels, 0); //加载图片
    if (data)
    {
    ...
    unsigned int specularMap; //纹理ID
    //
    // texture 2
    // ----------------------------------
    glGenTextures(1, &specularMap); //生成 1 个纹理，保存ID到texture
    glBindTexture(GL_TEXTURE_2D, specularMap); // 绑定纹理，接下来所有GL_TEXTURE_2D操作都是对此纹理
    // 设置纹理环绕方式参数 set the texture wrapping parameters
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); // GL_REPEAT (default wrapping method)
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    // 设置纹理过滤参数 set texture filtering parameters
...
//    int width, height, nrChannels;
    //    stbi_set_flip_vertically_on_load(true); // 让stb_image.h在加载图片时翻转y轴
    data = stbi_load("/书/OGL_Test/container2_specular.jpg", &width, &height, &nrChannels, 0); //加载图片
    if (data)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data); //生成一个纹理
        //参数：纹理目标GL_TEXTURE_2D，Mipmap级别0，纹理存储为RGB格式，宽度，高度，历史遗留总是0，使用RGB值加载，储存为char(byte)数组，图像数据
...
//        lightColor.z = sin(glfwGetTime() * 1.3f);
        ...
        // 材质属性 material properties（物体在三种光照下反射的颜色）
//        lightingShader.setVec3("material.ambient",  1.0f, 0.5f, 0.31f);
//        lightingShader.setVec3("material.diffuse",  1.0f, 0.5f, 0.31f);
//        lightingShader.setVec3("material.specular", 0.5f, 0.5f, 0.5f); // 改用贴图了
