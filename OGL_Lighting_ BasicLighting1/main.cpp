...
//    glGenBuffers(1, &EBO); //创建1个 索引缓冲对象
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);//忽略后三个法向量数据
...
//    ourShader.setInt("texture2", 1); // 使用着色器类设置
    ...
    // 由于投影矩阵很少改变，因此通常最好将它设置在主循环之外一次。
//    glm::mat4 projection = glm::perspective(glm::radians(45.0f), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f); // 投影矩阵
...
//        glBindTexture(GL_TEXTURE_2D, texture2); //绑定纹理
        ...
        lightingShader.setVec3("objectColor", 1.0f, 0.5f, 0.31f); //设定uniform（物体颜色）
        lightingShader.setVec3("lightColor",  1.0f, 1.0f, 1.0f); //设定uniform（光源颜色）
        lightingShader.setVec3("lightPos", lightPos); //设定uniform（光源位置）
