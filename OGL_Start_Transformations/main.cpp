...
int main()
{
    ...
    glUniform1i(glGetUniformLocation(ourShader.ID, "texture1"), 0);//告诉OpenGL每个着色器采样器属于哪个纹理单元
    ...
    while (!glfwWindowShouldClose(window)) //每次循环的开始前检查一次GLFW是否被要求退出
    {
        ...
        // 创建矩阵转换 create transformations
        glm::mat4 transform = glm::mat4(1.0f); // 矩阵初始化（单位矩阵）
        transform = glm::translate(transform, glm::vec3(1.0f, -0.5f, 0.0f)); // 3. transform * 位移矩阵
//        transform = glm::rotate(transform, glm::radians(90.0f), glm::vec3(0.0, 0.0, 1.0));//2.transform * 旋转矩阵
        transform = glm::rotate(transform, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f));//2.transform * 旋转矩阵
        transform = glm::scale(transform, glm::vec3(0.5, 0.5, 0.5)); // 1. transform * 缩放矩阵
        
        
        ourShader.use();
        unsigned int transformLoc = glGetUniformLocation(ourShader.ID, "transform"); // 查询uniform变量的地址
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(transform)); //把矩阵数据发送给着色器
        //参数：uniform的位置值，1个矩阵，不矩阵置换，矩阵数据（value_ptr来变换为OpenGL所希望接受的数据）
