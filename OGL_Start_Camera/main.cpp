...
//    glEnableVertexAttribArray(2); //启用顶点属性2（顶点的 属性位置 = 2）
    ...
    while (!glfwWindowShouldClose(window)) //每次循环的开始前检查一次GLFW是否被要求退出
    {
        ...
        float radius = 10.0f;
        float camX   = sin(glfwGetTime()) * radius; //摄像机在场景中旋转（圆：x^2 + z^2 = 10）
        float camZ   = cos(glfwGetTime()) * radius;
        view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));
        view = glm::lookAt(glm::vec3(camX, 0.0f, camZ), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));//变换到相机空间
        // 创建一个LookAt矩阵（参数：一个摄像机位置，一个目标位置，一个表示世界空间中的上向量的向量）
